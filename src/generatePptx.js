// generatePptx.js — NotebookLM-style, 2 files: EN + ZH
// Following Anthropic official pptxgenjs skill best practices
...
const C = {
  ...
  dark: '1A1A2E', darkGray: '333333', midGray: '888888',
  lightGray: 'E2E8F0', accent: '2563EB', accentLight: 'DBEAFE',
  accentDark: '1E3A5F',
};
const F = { main: 'Arial', cjk: 'Microsoft JhengHei' };
const LAYOUT = 'LAYOUT_WIDE'; // 13.33 x 7.5
...
function buildPptx(articles, overallInsights, weekRange, lang) {
  ...
  pptx.title = isZh ? '大腸直腸外科文獻週報' : 'Colorectal Surgery Weekly Digest';
  ...
  if (isZh) {
    ...
    cover.addText([
      { text: 'Colorectal Surgery', options: { fontSize: 44, color: C.white, bold: true, fontFace: F.main, breakLine: true } },
      { text: '大腸直腸外科文獻週報', options: { fontSize: 22, color: C.accentLight, fontFace: F.cjk } },
    ], { x: 0.8, y: 1.5, w: 11, lineSpacingMultiple: 1.3 });
  } else {
    cover.addText('WEEKLY DIGEST', { x: 0.8, y: 0.5, w: 6, fontSize: 9, color: C.midGray, charSpacing: 6, bold: true, margin: 0 });
    cover.addText('Colorectal\nSurgery', { x: 0.8, y: 1.5, w: 11, fontSize: 48, color: C.white, bold: true, fontFace: F.main, lineSpacingMultiple: 0.95 });
  }
  ...
  oSlide.addText(isZh ? 'Overall Insights\n& Future Directions' : 'Overall Insights\n& Future Directions', { x: 0.8, y: 0.9, w: 11, fontSize: 30, color: C.white, bold: true, fontFace: F.main, lineSpacingMultiple: 1.0 });
  if (isZh) {
    oSlide.addText('綜合啟示與未來研究方向', { x: 0.8, y: 2.5, w: 11, fontSize: 14, color: C.midGray, fontFace: F.cjk, margin: 0 });
  }
  ...
  back.addText(isZh ? 'Thank You' : 'Thank You', { x: 0.8, y: 2.5, w: 11, fontSize: 48, color: C.white, bold: true, fontFace: F.main, margin: 0 });
  if (isZh) {
    back.addText('謝謝', { x: 0.8, y: 4.1, w: 11, fontSize: 24, color: C.midGray, fontFace: F.cjk, margin: 0 });
  }
  back.addText('Colorectal Surgery Weekly Digest', { x: 0.8, y: isZh ? 5.0 : 4.3, w: 11, fontSize: 11, color: C.midGray, margin: 0 });

  return pptx;
}

// ════════════════════════════
// Public: generate both EN + ZH
// ════════════════════════════
