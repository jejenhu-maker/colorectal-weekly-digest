...
/**
 * Render articles into an HTML email body.
    ...
    <div style="border:1px solid #e2e8f0;border-radius:8px;padding:18px 20px;margin-bottom:18px;">
      <table cellpadding="0" cellspacing="0" border="0" width="100%"><tr>
        <td width="36" valign="top">
          <div style="background:#2563eb;color:white;font-weight:bold;font-size:14px;width:28px;height:28px;border-radius:50%;text-align:center;line-height:28px;">${num}</div>
        </td>
        <td valign="top">
          <a href="${link}" style="font-size:15px;font-weight:bold;color:#1e3a5f;text-decoration:none;" target="_blank">${a.title}</a>
        </td>
      </tr></table>
      ...
      <div style="font-size:13px;color:#475569;text-align:justify;line-height:1.6;background:#f8fafc;padding:8px 12px;border-radius:4px;border-left:3px solid #2563eb;">${a.abstractZh || '（無中文摘要）'}</div>

      ${a.doi ? `<div style="font-size:12px;margin-top:8px;"><a href="https://doi.org/${a.doi}" style="color:#2563eb;" target="_blank">DOI: ${a.doi}</a></div>` : ''}
      ${a.pmid ? `<div style="font-size:12px;margin-top:4px;"><a href="https://pubmed.ncbi.nlm.nih.gov/${a.pmid}/" style="color:#2563eb;" target="_blank">PubMed: ${a.pmid}</a></div>` : ''}
    </div>`;
  }).join('\n');

  // Page URL (GitHub Pages)
  const pageUrl = process.env.PAGE_URL || 'https://jejenhu-maker.github.io/colorectal-weekly-digest/';
...
<body style="font-family:'Helvetica Neue',Arial,'Noto Sans TC','Microsoft JhengHei',sans-serif;max-width:700px;margin:0 auto;padding:20px;color:#1a1a1a;">
  ...
  <div style="text-align:center;padding:40px 20px 30px;border-bottom:3px solid #2563eb;margin-bottom:30px;">
    <h1 style="font-size:24px;color:#1e3a5f;margin:0 0 8px;">🔬 Colorectal Surgery Weekly Digest</h1>
    <div style="font-size:16px;color:#64748b;">大腸直腸外科文獻週報</div>
    <div style="font-size:14px;color:#94a3b8;margin-top:12px;">${weekStart} — ${weekEnd}</div>
    <div style="margin-top:16px;">
      <a href="${pageUrl}" target="_blank" style="display:inline-block;background:#2563eb;color:white;text-decoration:none;padding:10px 24px;border-radius:8px;font-size:14px;font-weight:600;">📊 選文章 → 產生簡報 (.pptx)</a>
    </div>
  </div>
  ...
  <div style="text-align:center;font-size:12px;color:#94a3b8;margin-top:40px;padding-top:16px;border-top:1px solid #e2e8f0;">
    ...
    © ${now.year()} Colorectal Weekly Digest
  </div>
