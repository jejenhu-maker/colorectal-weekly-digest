// generateInsights.js — LLM: bullet points + clinical insight + future direction per article
...
async function generateInsights(articles) {
  ...
  // Individual article insights (3 at a time)
  ...
  try {
    const res = await getClient().chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `你是大腸直腸外科的資深研究學者。根據本週論文，用中英對照撰寫：
  ...
  try {
    const res = await getClient().chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `你是大腸直腸外科的資深研究學者。請針對以下論文生成結構化摘要。
