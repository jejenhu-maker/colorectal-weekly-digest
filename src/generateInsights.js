// generateInsights.js — use LLM to generate insights & future directions for each article
 ...
 *   - insight (EN + ZH): what this paper means for clinical practice
 *   - futureDirection (EN + ZH): possible future research
 * Also generate an overall synthesis for all articles combined.
 */
async function generateInsights(articles) {
  console.log(`[Insights] Generating insights for ${articles.length} articles…`);

  // Individual insights (batch 3 at a time)
  ...
  try {
    const res = await getClient().chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `你是大腸直腸外科的資深研究學者。請根據以下本週發表的論文，用中英對照的方式撰寫：
  ...
  try {
    const res = await getClient().chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `你是大腸直腸外科的資深研究學者。針對以下論文，請用中英對照撰寫：
